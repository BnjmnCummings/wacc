...
inline def defaultOverflow: A_Func = {
    val program: ListBuffer[A_Instr] = ListBuffer()
    program += A_And(A_Reg(A_RegName.StackPtr), A_Imm(STACK_ALIGN_VAL), PTR_SIZE)
    program += A_Lea(A_Reg(A_RegName.Arg1), A_MemOffset(A_Reg(A_RegName.InstrPtr), A_OffsetLbl(A_DataLabel(OVERFLOW_LBL_STR_NAME))))
...
inline def defaultPrintln: A_Func = {
    ...
    program += A_MovTo(A_Reg(A_RegName.BasePtr), A_Reg(A_RegName.StackPtr), PTR_SIZE)
    program += A_And(A_Reg(A_RegName.StackPtr), A_Imm(STACK_ALIGN_VAL), PTR_SIZE)
    program += A_Lea(A_Reg(A_RegName.Arg1), A_MemOffset(A_Reg(A_RegName.InstrPtr), A_OffsetLbl(A_DataLabel(PRINTLN_LBL_STR_NAME))))
...
inline def defaultPrintb: A_Func = {
    ...
    program += A_Lea(A_Reg(A_RegName.Arg3), A_MemOffset(A_Reg(A_RegName.InstrPtr), A_OffsetLbl(A_DataLabel(PRINTB_FALSE_LBL_STR_NAME))))
    program += A_Jmp(A_InstrLabel(PRINTB_TRUE_LABEL), A_Cond.Uncond)

    program += A_LabelStart(A_InstrLabel(PRINTB_FALSE_LABEL))
    program += A_Lea(A_Reg(A_RegName.Arg3), A_MemOffset(A_Reg(A_RegName.InstrPtr), A_OffsetLbl(A_DataLabel(PRINTB_TRUE_LBL_STR_NAME))))
    ...
    program += A_Lea(A_Reg(A_RegName.Arg1), A_MemOffset(A_Reg(A_RegName.InstrPtr), A_OffsetLbl(A_DataLabel(PRINTB_LBL_STR_NAME))))
    program += A_MovTo(A_Reg(A_RegName.RetReg), A_Imm(ZERO_IMM), BYTE_SIZE)
    program += A_Call(A_ExternalLabel(PRINTF))
    program += A_MovTo(A_Reg(A_RegName.Arg1), A_Imm(ZERO_IMM), PTR_SIZE)
    program += A_Call(A_ExternalLabel(F_FLUSH))
...
inline def defaultPrints: A_Func = {
    ...
    program += A_Lea(A_Reg(A_RegName.Arg1), A_MemOffset( A_Reg(A_RegName.InstrPtr), A_OffsetLbl(A_DataLabel(PRINTS_LBL_STR_NAME))))
    program += A_MovTo(A_Reg(A_RegName.RetReg), A_Imm(ZERO_IMM), BYTE_SIZE)
    program += A_Call(A_ExternalLabel(PRINTF))
    program += A_MovTo(A_Reg(A_RegName.Arg1), A_Imm(ZERO_IMM), PTR_SIZE)
    program += A_Call(A_ExternalLabel(F_FLUSH))
...
inline def defaultReadc: A_Func = {
    ...
    program += A_MovDeref(A_RegDeref(A_MemOffset(A_Reg(A_RegName.StackPtr), A_OffsetImm(ZERO_IMM))), A_Reg(A_RegName.Arg1), CHAR_SIZE)
    program += A_Lea(A_Reg(A_RegName.Arg2), A_MemOffset(A_Reg(A_RegName.StackPtr), A_OffsetImm(0)))
    program += A_Lea(A_Reg(A_RegName.Arg1), A_MemOffset(A_Reg(A_RegName.InstrPtr), A_OffsetLbl(A_DataLabel(READC_LBL_STR_NAME))))
    program += A_MovTo(A_Reg(A_RegName.RetReg), A_Imm(ZERO_IMM), BYTE_SIZE)
...
inline def defaultReadi: A_Func = {
    ...
    program += A_And(A_Reg(A_RegName.StackPtr), A_Imm(STACK_ALIGN_VAL), PTR_SIZE)
    program += A_Sub(A_Reg(A_RegName.StackPtr), A_Imm(16), PTR_SIZE)
    program += A_MovDeref(A_RegDeref(A_MemOffset(A_Reg(A_RegName.StackPtr), A_OffsetImm(ZERO_IMM))), A_Reg(A_RegName.Arg1), INT_SIZE)
    program += A_Lea(A_Reg(A_RegName.Arg2), A_MemOffset(A_Reg(A_RegName.StackPtr), A_OffsetImm(0)))
    program += A_Lea(A_Reg(A_RegName.Arg1), A_MemOffset(A_Reg(A_RegName.InstrPtr), A_OffsetLbl(A_DataLabel(READI_LBL_STR_NAME))))
    program += A_MovTo(A_Reg(A_RegName.RetReg), A_Imm(ZERO_IMM), BYTE_SIZE)
    program += A_Call(A_ExternalLabel(SCANF))
...
inline def defaultBadChar: A_Func = {
    ...
    program += A_And(A_Reg(A_RegName.StackPtr), A_Imm(STACK_ALIGN_VAL), PTR_SIZE)
    program += A_Lea(A_Reg(A_RegName.Arg1), A_MemOffset(A_Reg(A_RegName.InstrPtr), A_OffsetLbl(A_DataLabel(ERR_BAD_CHAR_STR_NAME))))
    program += A_MovTo(A_Reg(A_RegName.RetReg), A_Imm(ZERO_IMM), BYTE_SIZE)
    program += A_Call(A_ExternalLabel(PRINTF))
    program += A_MovTo(A_Reg(A_RegName.Arg1), A_Imm(ZERO_IMM), PTR_SIZE)
    program += A_Call(A_ExternalLabel(F_FLUSH))
...
inline def defaultDivZero: A_Func = {
    ...
    program += A_And(A_Reg(A_RegName.StackPtr), A_Imm(STACK_ALIGN_VAL), PTR_SIZE)
    program += A_Lea(A_Reg(A_RegName.Arg1), A_MemOffset(A_Reg(A_RegName.InstrPtr), A_OffsetLbl(A_DataLabel(DIV_ZERO_LBL_STR_NAME))))
    program += A_Call(A_InstrLabel(PRINTS_LABEL))
    program += A_MovTo(A_Reg(A_RegName.Arg1), A_Imm(ERR_EXIT_CODE), BYTE_SIZE)
    program += A_Call(A_ExternalLabel(EXIT))
...
inline def defaultOutOfBounds: A_Func = {
    ...
    program += A_And(A_Reg(A_RegName.StackPtr), A_Imm(STACK_ALIGN_VAL), PTR_SIZE)
    program += A_Lea(A_Reg(A_RegName.Arg1), A_MemOffset(A_Reg(A_RegName.InstrPtr), A_OffsetLbl(A_DataLabel(OUT_OF_BOUNDS_LBL_STR_NAME))))
    ...
    program += A_MovTo(A_Reg(A_RegName.Arg1), A_Imm(ERR_EXIT_CODE), BOOL_SIZE) 
    program += A_Call(A_ExternalLabel(EXIT))
...
inline def defaultOutOfMemory: A_Func = {
    ...
    program += A_And(A_Reg(A_RegName.StackPtr), A_Imm(STACK_ALIGN_VAL), PTR_SIZE)
    program += A_Lea(A_Reg(A_RegName.Arg1), A_MemOffset(A_Reg(A_RegName.InstrPtr), A_OffsetLbl(A_DataLabel(OUT_OF_MEMORY_LBL_STR_NAME))))
    program += A_Call(A_InstrLabel(PRINTS_LABEL))
    program += A_MovTo(A_Reg(A_RegName.Arg1), A_Imm(ERR_EXIT_CODE), BOOL_SIZE)
    program += A_Call(A_ExternalLabel(EXIT))
...
inline def defaultErrNull: A_Func = {
    ...
    program += A_And(A_Reg(A_RegName.StackPtr), A_Imm(STACK_ALIGN_VAL), PTR_SIZE)
    program += A_Lea(A_Reg(A_RegName.Arg1), A_MemOffset(A_Reg(A_RegName.InstrPtr), A_OffsetLbl(A_DataLabel(ERR_NULL_PAIR_STR_NAME))))
    program += A_Call(A_InstrLabel(PRINTS_LABEL))
    program += A_MovTo(A_Reg(A_RegName.Arg1), A_Imm(ERR_EXIT_CODE), BYTE_SIZE)
    program += A_Call(A_ExternalLabel(EXIT))
