...
private def genIntLiteral(v: BigInt)(using ctx: CodeGenCtx): List[A_Instr] = 
    List(A_Mov(A_Reg(A_RegName.RetReg), A_Imm(v), INT_SIZE))

private def genBoolLiteral(v: Boolean)(using ctx: CodeGenCtx): List[A_Instr] = 
    List(A_Mov(A_Reg(A_RegName.RetReg), A_Imm(if v then TRUE else FALSE), BOOL_SIZE))

private def genCharLiteral(v: Char)(using ctx: CodeGenCtx): List[A_Instr] = 
...
private def funcLabelGen(f: Name): A_InstrLabel = 
