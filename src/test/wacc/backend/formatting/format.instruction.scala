...
class format_arithmetic_op_test extends AnyFlatSpec {
    "formatInstr" should "be able to format an add instruction" in {
        formatInstr(A_Add(A_Reg(A_OperandSize.A_32, A_RegName.RetReg), A_Imm(5), A_OperandSize.A_32)) should be ("add eax, 5")
    }

    it should "be able to format a sub instruction" in {
        formatInstr(A_Sub(A_Reg(A_OperandSize.A_32, A_RegName.RetReg), A_Imm(5), A_OperandSize.A_32)) should be ("sub eax, 5")
    }

    it should "be able to format a mul instruction" in {
        formatInstr(A_Mul(A_Reg(A_OperandSize.A_32, A_RegName.RetReg), A_Imm(5), A_OperandSize.A_32)) should be ("mul eax, 5")
    }

    it should "be able to format a div instruction" in {
        formatInstr(A_Div(A_Reg(A_OperandSize.A_32, A_RegName.RetReg), A_Imm(5), A_OperandSize.A_32)) should be ("div eax, 5")
    }
}

class format_branching_test extends AnyFlatSpec {
    "formatInstr" should "be able to format a cmp instruction" in {
        formatInstr(A_Cmp(A_Reg(A_OperandSize.A_32, A_RegName.RetReg), A_Imm(5), A_OperandSize.A_32)) should be ("cmp eax, 5")
    }
...
class format_memory_ops_test extends AnyFlatSpec {
    "formatInstr" should "be able to format a push instruction" in {
        formatInstr(A_Push(A_Reg(A_OperandSize.A_32, A_RegName.RetReg))) should be ("push eax")
    }

    it should "be able to format a pop instruction" in {
        formatInstr(A_Pop(A_Reg(A_OperandSize.A_32, A_RegName.RetReg))) should be ("pop eax")
    }

    it should "be able to format a mov instruction" in {
        formatInstr(A_Mov(A_Reg(A_OperandSize.A_32, A_RegName.RetReg), A_Imm(5))) should be ("mov eax, 5")
    }

    it should "be able to format a lea instruction" in {
        formatInstr(A_Lea(
            A_Reg(A_OperandSize.A_32, A_RegName.RetReg), 
            A_MemOffset(A_OperandSize.A_32, A_Reg(A_OperandSize.A_32, A_RegName.RetReg), A_OffsetImm(5)))
